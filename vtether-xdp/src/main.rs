...
use conntrack::{
    ct_create4, ct_lazy_lookup4, CtState, CtStatus, Ipv4CtTuple,
...
fn try_xdp(ctx: &XdpContext) -> Result<u32, ()> {
    ...
    // If the packet's source IP is our SNAT IP, this is a forwarded packet
    // that re-entered XDP. Pass it to the kernel stack for actual TX.
...
fn handle_forward(
    ...
    // Re-parse + stats
    let (ip2, _) = parse::parse_ipv4_tcp_validated(ctx)?;
    let pkt_len = u16::from_be(read_field(unsafe { addr_of!((*ip2).tot_len) })) as u64;
    stats::update_route_stats(svc.rev_nat_index, pkt_len, is_new);

    // Let the kernel stack forward the rewritten packet.
    // On virtio_net, bpf_redirect to the same interface re-enters XDP causing
    // a loop. Instead, XDP_PASS delivers the DNAT+SNAT'd packet to the kernel,
    // which forwards it via ip_forward (since dst is not a local IP).
...
fn handle_reply(
    ...
    let (ip2, l4_off2) = parse::parse_ipv4_tcp_validated(ctx)?;

    let backend_ip = read_field(unsafe { addr_of!((*ip2).saddr) });
    let backend_port = read_field(
        parse::ptr_at::<u16>(ctx, l4_off2 + parse::TCP_SPORT_OFF)? as *const u16,
    );
    ...
    let mut ct_state = CtState::new();
    match ct_lazy_lookup4(ctx, &rev_lookup_tuple, l4_off2, CT_INGRESS, &mut ct_state)? {
        CtStatus::New => {
    ...
    if ct_state.rev_nat_index != 0 {
        lb::lb4_rev_nat(ctx, ip2, l4_off2, ct_state.rev_nat_index)?;
        info!(ctx, "REPLY: revDNAT done");
    }

    let (ip3, _) = parse::parse_ipv4_tcp_validated(ctx)?;
    let pkt_len = u16::from_be(read_field(unsafe { addr_of!((*ip3).tot_len) })) as u64;
    if ct_state.rev_nat_index != 0 {
        stats::update_route_stats(ct_state.rev_nat_index, pkt_len, false);
    }

    // Pass to kernel stack for routing (same reason as forward path —
    // bpf_redirect to the same interface re-enters XDP on virtio_net).
