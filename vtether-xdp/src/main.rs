...
fn handle_forward(
    ...
    let (backend_id, is_new) =
        match ct_lazy_lookup4(tcp_flags, &fwd_tuple, CT_SERVICE, &mut ct_state) {
            CtStatus::New => {
                ...
                if backend_id == 0 {
                    debug_warn!(ctx, "FWD: backend slot lookup returned 0");
                    stats::update_route_drops(svc.rev_nat_index);
                    return Ok(drop);
                }
            ...
            CtStatus::Established | CtStatus::Reply => {
                ...
                if backend_id == 0 {
                    debug_warn!(ctx, "FWD: CT established but backend_id=0");
                    stats::update_route_drops(svc.rev_nat_index);
                    return Ok(drop);
                }
                (backend_id, false)
    ...
    let backend = match lb4_lookup_backend(backend_id) {
        Some(be) => be,
        None => {
            debug_error!(ctx, "FWD: backend_id={} not found in LB4_BACKENDS", backend_id);
            return Err(());
        }
    };
    ...
    // DNAT
    if lb::lb4_xlate_dnat(ctx, ip, l4_off, old_daddr, backend.address, old_dport, backend.port)
        .is_err()
    {
        debug_error!(ctx, "FWD: DNAT rewrite failed (bounds check)");
        return Err(());
    }
    ...
    let snat_cfg = match SNAT_CONFIG.get(0) {
        Some(cfg) => cfg,
        None => {
            debug_error!(ctx, "FWD: SNAT_CONFIG map empty");
            return Err(());
        }
    };
    ...
    let snat_port = match nat::snat_v4_nat(
        ...
        Ok(port) => port,
        Err(()) => {
            debug_error!(ctx, "FWD: SNAT failed (port exhaustion or bounds check)");
            return Err(());
        }
    };
...
fn handle_reply(
    ...
    if ct_state.rev_nat_index != 0 {
        if lb::lb4_rev_nat(ctx, ip, l4_off, ct_state.rev_nat_index).is_err() {
            debug_error!(
                ctx,
                "REPLY: revDNAT failed for rev_nat_index={}",
